    ...
    try:
        ...
        if(args.outputFilePath != None and args.outputFilePath != ''):
            ...
            for video in retVal:
                ...
                if count == 1:
                    videoElement = deepcopy(contentVideoGrid['nodes'][1])
                    contentVideoGrid.replace('VideoId1', videoId)
                    contentVideoGrid.replace('#VidoeTitle1', title)
                    contentVideoGrid.replace('#VideoDescription1', description)
                else:
                   if count == 2:
                    contentVideoGrid.replace('VideoId2', videoId)
                    contentVideoGrid.replace('#VidoeTitle2', title)
                    contentVideoGrid.replace('#VideoDescription2', description)
                   else:
                       if count == 3:
                        contentVideoGrid.replace('VideoId3', videoId)
                        contentVideoGrid.replace('#VidoeTitle3', title)
                        contentVideoGrid.replace('#VideoDescription3', description)
                       else:
                           if count == 4:
                            contentVideoGrid.replace('VideoId4', videoId)
                            contentVideoGrid.replace('#VidoeTitle4', title)
                            contentVideoGrid.replace('#VideoDescription4', description)
                           else:
                            videoElement2 = deepcopy(videoElement)
                            contentVideoGrid['nodes'].append(videoElement2)
                            contentVideoGrid.replace('VideoId1', videoId)
                            contentVideoGrid.replace('#VidoeTitle1', title)
                            contentVideoGrid.replace('#VideoDescription1', description)
